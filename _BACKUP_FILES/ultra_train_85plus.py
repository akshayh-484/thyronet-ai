"""
ULTRA TRAINING - FINAL PUSH FOR 85%+
Combining best techniques + hyperparameter tuning
"""
...
print("="*80)
print("🔥 ULTRA TRAINING - FINAL PUSH FOR 85%+")
print("="*80)
print("Strategy:")
print("  • Best preprocessing: BorderlineSMOTE + MinMaxScaler")
print("  • Hyperparameter tuning for top 3 models")
print("  • Longer training (5000+ iterations)")
print("  • Weighted ensemble of best models")
...
# Train models with ULTRA parameters
print("\n" + "="*80)
...
# 1. CatBoost - ULTRA
print("\n1️⃣  CatBoost (ULTRA - 5000 iterations)...")
...
models['CatBoost_Ultra'] = catboost
print(f"   ✅ Accuracy: {results['CatBoost_Ultra']['accuracy']*100:.2f}%")

# 2. XGBoost - ULTRA
print("\n2️⃣  XGBoost (ULTRA - 5000 iterations)...")
xgb = XGBClassifier(
    n_estimators=5000,
    max_depth=25,
    learning_rate=0.001,
...
models['XGBoost_Ultra'] = xgb
print(f"   ✅ Accuracy: {results['XGBoost_Ultra']['accuracy']*100:.2f}%")

# 3. LightGBM - ULTRA
print("\n3️⃣  LightGBM (ULTRA - 5000 iterations)...")
lgbm = LGBMClassifier(
    n_estimators=5000,
    max_depth=25,
    learning_rate=0.001,
...
models['LightGBM_Ultra'] = lgbm
print(f"   ✅ Accuracy: {results['LightGBM_Ultra']['accuracy']*100:.2f}%")

# 4. Weighted Ensemble
print("\n4️⃣  Weighted Ensemble (All 3 models)...")
weights = [
    results['CatBoost_Ultra']['accuracy'],
    results['XGBoost_Ultra']['accuracy'],
    results['LightGBM_Ultra']['accuracy']
]
total_weight = sum(weights)
weights = [w/total_weight for w in weights]

ensemble_proba = (
    models['CatBoost_Ultra'].predict_proba(X_test_scaled) * weights[0] +
    models['XGBoost_Ultra'].predict_proba(X_test_scaled) * weights[1] +
    models['LightGBM_Ultra'].predict_proba(X_test_scaled) * weights[2]
)
...
if best_model_name == 'Weighted_Ensemble':
    best_model = {
        ...
        'models': {
            'CatBoost': models['CatBoost_Ultra'],
            'XGBoost': models['XGBoost_Ultra'],
            'LightGBM': models['LightGBM_Ultra']
        },
        'weights': weights
    }
else:
